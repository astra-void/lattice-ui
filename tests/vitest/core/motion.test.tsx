...
describe("core motion helpers", () => {
  ...
  it("transitions between phases and sets goals", () => {
    ...
    expect(tweenService.createdTweens[0]?.goals.Position).toEqual(UDim2.fromOffset(0, 0));
    ...
    expect(tweenService.createdTweens).toHaveLength(2);
    expect(tweenService.createdTweens[1]?.goals.Position).toEqual(UDim2.fromOffset(0, 12));
  });
  ...
  it("short-circuits tween creation when motion is disabled and applies goals immediately", () => {
    ...
    expect(tweenService.createdTweens).toHaveLength(0);
