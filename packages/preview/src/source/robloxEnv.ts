...
export interface UniversalRobloxMock {
  (...args: unknown[]): UniversalRobloxMock;
...
function createUniversalRobloxMockInternal(path: MockPath = []): UniversalRobloxMock {
  ...
  const resolveMember = (key: PropertyKey) => {
    ...
    const nextMember = createUniversalRobloxMockInternal([...path, key]);
    members.set(key, nextMember);
    return nextMember;
  };

  proxy = new Proxy(callableTarget as UniversalRobloxMock, {
    apply() {
      return proxy;
    },
    construct() {
